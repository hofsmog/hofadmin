...
export default async function FormsListPage({ searchParams }: { searchParams?: Promise<{ status?: string }> }) {
  ...
  return (
    <>
      <ModuleHeader
        title="Formulär"
        description="Formulär och undersökningar med publicering, delning och svar."
        items={formsNavItems}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between gap-4">
            <div>
              <CardTitle>Formbibliotek</CardTitle>
              <CardDescription>Hantera formulär, undersökningar och publika länkar.</CardDescription>
            </div>
        ...
        <form className="flex flex-col gap-3 p-5 pt-0 sm:flex-row sm:items-center">
          ...
          <button className="h-11 rounded-xl bg-zinc-950 px-4 text-sm font-medium text-white transition hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-950" type="submit">Filtrera</button>
        </form>
        <div className="space-y-3 p-5 pt-0">
          {(forms ?? []).length ? forms?.map((form) => {
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <ButtonLink href={form.form_type === "survey" ? `/dashboard/forms/${form.id}/results` : `/dashboard/forms/submissions?formId=${form.id}`} variant="secondary" className="h-9 px-3">
                        ...
                        {form.form_type === "survey" ? "Sammanställning" : "Inkorg"}
                      </ButtonLink>
            ...
            <div className="rounded-xl border border-dashed p-8 text-center">
              ...
              <p className="mt-3 font-medium">Inga formulär ännu</p>
              <p className="mt-1 text-sm text-muted-foreground">Skapa ett formulär eller en undersökning för att börja samla in svar.</p>
            </div>
          )}
        </div>
      </Card>
